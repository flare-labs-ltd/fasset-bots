...
export class FuzzingTimeline {
    ...
    async skipTime(seconds: number) {
        ...
        this.runner.comment(`***** BLOCKS  flare=${(await web3.eth.getBlock("latest")).number}, ${await latestBlockTimestamp()}  chain=${await this.chain.getCurrentBlockHeight()}, ${this.chain.lastBlockTimestamp()}`);
    }
}
