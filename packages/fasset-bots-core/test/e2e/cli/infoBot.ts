...
describe("InfoBot cli commands unit tests", () => {
    ...
    it("Should create InfoBot", async () => {
        const infoBot1 = await InfoBotCommands.create(FASSET_BOT_CONFIG, "FfakeXRP");
        ...
        const infoBot2 = await InfoBotCommands.create(FASSET_BOT_CONFIG);
    ...
    it("Should not create InfoBot", async () => {
        await expect(InfoBotCommands.create(FASSET_BOT_CONFIG, "Invalid")).to.eventually.be.rejectedWith("FAsset does not exist");
    });
});
