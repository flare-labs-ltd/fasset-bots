...
export class TransactionService {
    ...
    async preparePaymentTransaction(
        ...
        if (feeSource && amountInSatoshi && !freeUnderlying) {
            return this.preparePaymentTransactionWithAdditionalFeeWallet(txDbId, source, feeSource, destination, amountInSatoshi, feeInSatoshi, note, txForReplacement, undefined, securityFeePerKB);
            ...
            return this.preparePaymentTransactionWithSingleWallet(txDbId, source, destination, amountInSatoshi, feeInSatoshi, note, txForReplacement, securityFeePerKB)
        }
    }
    ...
    async preparePaymentTransactionWithSingleWallet(
        ...
        const feePerKB = maxBN(feePerKBFromFeeService, securityFeePerKB ?? toBN(0));
        const txData = {
            source: source,
    ...
    async preparePaymentTransactionWithAdditionalFeeWallet(
        ...
        const txDataForFee = {
            source: feeSource,
            destination: destination,
            amount: toBN(baseTransaction.getFee()), //TODO if feeInSatoshi? should it be feeInSatoshi?
            fee: toBN(0),
            feePerKB: feePerKB,
            useChange: false,
            note: note,
        ...
        if (utxosForFee.length === 0) {
            ...
            const feeAmount = utxosForFee.reduce((accumulator, utxo) => accumulator.add(utxo.value), new BN(0));
            const txData = {
                source: source,
                destination: destination,
        ...
        if (freeUnderlying && utxosForFeeAmount.ltn(tr.getFee()) && !txForReplacement) {
            await this.correctFeeForFreeUnderlying(tr, utxos, utxosForFeeAmount, source, destination, amountInSatoshi, note);
        }
