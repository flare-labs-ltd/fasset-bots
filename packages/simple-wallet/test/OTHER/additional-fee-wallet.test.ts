...
describe("Unit test for paying fees from additional wallet", () => {
    ...
    it("If 'fee' wallet has enough funds fee should be covered from it, fee should be covered from it if it's greater than dust 2", async () => {
        ...
        const [tr,] = await wClient.transactionService.preparePaymentTransactionWithAdditionalFeeWallet(0, fundedAddress, fundedFeeAddress, targetAddress, toBN(1500), feePerKB);
    ...
    it("If 'fee' wallet has enough funds fee should be covered from it if fee is greater than dust", async () => {
        ...
        const [tr,] = await wClient.transactionService.preparePaymentTransactionWithAdditionalFeeWallet(0, fundedAddress, fundedFeeAddress, targetAddress, toBN(1500), feePerKB, undefined);
    ...
    it("If 'fee' wallet has enough funds fee should be covered from it, remainder should be returned only if it's greater than dust", async () => {
        ...
        const [tr,] = await wClient.transactionService.preparePaymentTransactionWithAdditionalFeeWallet(0, fundedAddress, fundedFeeAddress, targetAddress, toBN(1500), feePerKB, undefined, toBN(600));
    ...
    it("RBF fee per byte should be >= original fee byte + relay fee per byte", async () => {
        ...
        const mempoolUTXOs = await wClient.transactionUTXOService.sortedMempoolUTXOs(fundedAddress);
