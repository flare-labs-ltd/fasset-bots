...
import fs from "fs";
import { CommandLineError, errorIncluded, squashSpace, toBIPS, toplevelRun } from "@flarelabs/fasset-bots-core/utils";
...
toplevelRun(async () => {
    try {
        ...
        if (errorIncluded(error, ["invalid agent vault address"])) {
