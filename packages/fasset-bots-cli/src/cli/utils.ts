...
async function setupContext(fAssetSymbol: string) {
    ...
    const walletHelper = createBlockchainWalletHelper(secrets, encodeAttestationName(chainConfig.chainId), orm.em, chainConfig.walletUrl, runConfig.walletOptions);
