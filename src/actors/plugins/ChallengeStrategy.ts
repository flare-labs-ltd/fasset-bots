...
export abstract class ChallengeStrategy {
    ...
    abstract illegalTransactionChallenge(
        scope: EventScope,
        agent: TrackedAgentState,
        proof: BalanceDecreasingTransaction.Proof
    ): Promise<any>;
    ...
    abstract freeBalanceNegativeChallenge(
        scope: EventScope,
        agent: TrackedAgentState,
        transactionHashes: BalanceDecreasingTransaction.Proof[]
    ): Promise<any>;
}

export class DefaultChallengeStrategy extends ChallengeStrategy {

    public async illegalTransactionChallenge(
        scope: EventScope,
        agent: TrackedAgentState,
        proof: BalanceDecreasingTransaction.Proof
    ): Promise<void> {
    ...
    public async freeBalanceNegativeChallenge(
        scope: EventScope,
        agent: TrackedAgentState,
        proofs: BalanceDecreasingTransaction.Proof[]
    ): Promise<void> {
        // due to async nature of challenging there may be some false challenges which will be rejected
        await this.state.context.assetManager
...
export class LiveChallengeStrategy extends ChallengeStrategy {
    ...
    public async illegalTransactionChallenge(
        ...
        await challenger.illegalPaymentChallenge(proof, agent.vaultAddress, { from: this.address })
            .catch((e) =>
                scope.exitOnExpectedError(e, [
                    "chlg: already liquidating",
                    "chlg: transaction confirmed",
    ...
    public async doublePaymentChallenge(
        ...
        // due to async nature of challenging there may be some false challenges which will be rejected
        const challenger = await Challenger.at(this.state.context.challengeStrategy!.config.address);
        await challenger.doublePaymentChallenge(proof1, proof2, agent.vaultAddress, { from: this.address })
            .catch((e) => scope.exitOnExpectedError(e, ["chlg dbl: already liquidating"]));
    }
    ...
    public async freeBalanceNegativeChallenge(
        ...
        // due to async nature of challenging there may be some false challenges which will be rejected
        const challenger = await Challenger.at(this.state.context.challengeStrategy!.config.address);
        await challenger.freeBalanceNegativeChallenge(proofs, agent.vaultAddress, { from: this.address })
            .catch((e) => scope.exitOnExpectedError(e, ["mult chlg: already liquidating", "mult chlg: enough balance"]));
    }
}