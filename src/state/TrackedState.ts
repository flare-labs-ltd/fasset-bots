...
import { BN_ZERO, fail, toBN } from "../utils/helpers";
...
export class TrackedState {
    ...
    registerHandlers() {
        ...
        this.assetManagerEvent('SettingChanged').subscribe(args => {
            if (!(args.name in this.settings)) fail(`Invalid setting change ${args.name}`);
        ...
        this.assetManagerEvent('SettingArrayChanged').subscribe(args => {
            if (!(args.name in this.settings)) fail(`Invalid setting array change ${args.name}`);
