...
export class AgentCollateral {
    ...
    static async create(assetManager: AssetManagerInstance, settings: AssetManagerSettings, agentVault: string): Promise<AgentCollateral> {
        ...
        const agentPoolTokenCD = await collateralDataFactory.agentPoolTokens(poolCD, collateralPoolToken, agentVault);
        return new AgentCollateral(settings, agentInfo, class1CD, poolCD, agentPoolTokenCD);
    }
