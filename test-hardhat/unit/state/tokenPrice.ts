...
describe("Token price unit tests", async () => {
    ...
    it("Should create TokenPrice", async () => {
        ...
        expect(tokenPrice.toNumber()).eq(0.123456);
        expect(tokenPrice.toFixed()).eq("0.123");
        expect(tokenPrice.toString()).eq("0.123");
        expect(tokenPrice.amgToTokenWei(await context.assetManager.getSettings(), tokenPrice.decimals, tokenPrice).toString()).eq("1000000000");
    });
