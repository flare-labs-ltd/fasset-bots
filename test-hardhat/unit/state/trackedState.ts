...
import { decodeLiquidationStrategyImplSettings, encodeLiquidationStrategyImplSettings, LiquidationStrategyImplSettings } from "../../../src/fasset/LiquidationStrategyImpl";
...
describe("Tracked state tests", async () => {
    ...
    it("Should create agent with current state", async () => {
        const agentBLocal = await createAgentB(context, accounts[0], underlyingAddress);
    ...
    it("Should handle event 'AgentCreated'", async () => {
        await createAgentB(context, accounts[0], underlyingAddress);
        expect(trackedState.agents.size).to.eq(0);
        await trackedState.readUnhandledEvents();
        expect(trackedState.agents.size).to.eq(1);
    });
    ...
    it("Should handle event 'AgentAvailable'", async () => {
        ...
        const agentBLocal = await createAgentB(context, ownerLocal, underlyingAddress);
    ...
    it("Should handle event 'SelfClose'", async () => {
        const agentBLocal = await createAgentB(context, ownerAddress, underlyingAddress);
    ...
    it("Should handle event 'CollateralReserved'", async () => {
        const agentB = await createAgentBAndMakeAvailable(context, ownerAddress, underlyingAddress);
